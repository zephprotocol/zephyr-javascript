...
const ZephyrBalance = require("./model/ZephyrBalance");
const ZephyrReserveInfo = require("./model/ZephyrReserveInfo");
...
class MoneroWalletFull extends MoneroWalletKeys {
  ...
  async getReserveInfo() {
    ...
    return that._module.queueTask(async function () {
      ...
      return new Promise(function (resolve, reject) {
        let callbackFn = function (resp) {
          const reserveInfo = new ZephyrReserveInfo(JSON.parse(GenUtils.stringifyBIs(resp)).reserve_info);
          resolve(reserveInfo);
        };
  ...
  async getBalance(accountIdx, subaddressIdx, asset_type) {
    ...
    return that._module.queueTask(async function () {
      ...
      return new ZephyrBalance(JSON.parse(GenUtils.stringifyBIs(balanceStr)).balance, asset_type);
    });
  }
  ...
  async getUnlockedBalance(accountIdx, subaddressIdx, asset_type) {
    ...
    return that._module.queueTask(async function () {
      ...
      // parse json string to BigInteger
      return new ZephyrBalance(JSON.parse(GenUtils.stringifyBIs(unlockedBalanceStr)).unlockedBalance, asset_type);
    });
  }
...
class MoneroWalletFullProxy extends MoneroWallet {
  ...
  async getReserveInfo() {
    ...
    return new ZephyrReserveInfo(reserveInfo);
  }

  async getBalance(accountIdx, subaddressIdx, asset_type) {
    let balanceStr = await this._invokeWorker("getBalance", Array.from(arguments));

    return new ZephyrBalance(await this._invokeWorker("getBalance", Array.from(arguments)));
  }

  async getUnlockedBalance(accountIdx, subaddressIdx, asset_type) {
    let unlockedBalanceStr = await this._invokeWorker("getUnlockedBalance", Array.from(arguments));
    return new ZephyrBalance(unlockedBalanceStr);
  }
