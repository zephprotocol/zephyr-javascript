...
class MoneroTxConfig {
   ...
   */
  constructor(config, relaxValidation) {
    // relax validation for internal use to process json from rpc or cpp
    if (arguments.length > 2)
      throw new MoneroError(
        "MoneroTxConfig can be constructed with only two parameters but was given " + arguments.length,
      );
    ...
    if (!config) this.state = {};
      ...
      if (relaxValidation && typeof this.state.amount === "number")
        this.state.amount = BigInteger.parse(this.state.amount);
    ...
    if (this.state.destinations) {
      ...
      this.setDestinations(
        this.state.destinations.map((destination) =>
          destination instanceof MoneroDestination ? destination : new MoneroDestination(destination),
        ),
      );
    }
   ...
   */
  setAddress(address) {
    if (this.state.destinations !== undefined && this.state.destinations.length > 1)
