...
class MoneroTxWallet extends MoneroTx {
  ...
  constructor(state) {
    ...
    if (state.incomingTransfers) {
      for (let i = 0; i < state.incomingTransfers.length; i++) {
        if (!(state.incomingTransfers[i] instanceof MoneroIncomingTransfer)) {
          state.incomingTransfers[i] = new MoneroIncomingTransfer(
            Object.assign(state.incomingTransfers[i], { tx: this }),
          );
        }
      }
    }
    ...
    if (state.inputSum !== undefined && !(state.inputSum instanceof BigInteger))
      state.inputSum = BigInteger.parse(state.inputSum);
    if (state.outputSum !== undefined && !(state.outputSum instanceof BigInteger))
      state.outputSum = BigInteger.parse(state.outputSum);
    if (state.changeAmount !== undefined && !(state.changeAmount instanceof BigInteger))
      state.changeAmount = BigInteger.parse(state.changeAmount);
  }
  ...
  filterTransfers(transferQuery) {
    ...
    if (this.getIncomingTransfers()) {
      ...
      this.setIncomingTransfers(
        this.getIncomingTransfers().filter(function (transfer) {
  ...
  filterOutputs(outputQuery) {
    ...
    if (super.getOutputs()) {
      ...
      this.setOutputs(
        super.getOutputs().filter(function (output) {
  ...
  static _mergeIncomingTransfer(transfers, transfer) {
    for (let aTransfer of transfers) {
      if (
        aTransfer.getAccountIndex() === transfer.getAccountIndex() &&
        aTransfer.getSubaddressIndex() === transfer.getSubaddressIndex()
      ) {
