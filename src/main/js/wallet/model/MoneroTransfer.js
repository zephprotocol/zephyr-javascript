...
class MoneroTransfer {
  ...
  constructor(state) {
    ...
    if (state.amount !== undefined && !(state.amount instanceof BigInteger))
      state.amount = BigInteger.parse(state.amount);
  ...
  setCurrency(asset_type) {
    this.state.asset_type = asset_type;
    return this;
  }

  getCurrency() {
    return this.state.asset_type;
  }
  ...
  merge(transfer) {
    ...
    if (
      ...
      transfer.getAmount() !== undefined &&
      this.getAmount().compare(transfer.getAmount()) !== 0 &&
      (this.getAmount().compare(BigInteger.parse("0")) === 0 ||
        transfer.getAmount().compare(BigInteger.parse("0")) === 0)
    ) {
