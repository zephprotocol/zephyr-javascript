...
self.getWasmMemoryUsed = async function (objectId) {
  return LibraryUtils.getWasmModule() && LibraryUtils.getWasmModule().HEAP8
    ? LibraryUtils.getWasmModule().HEAP8.length
    : undefined;
};
...
self.daemonGetOutputHistogram = async function (daemonId, amounts, minCount, maxCount, isUnlocked, recentCutoff) {
  let entriesJson = [];
  for (let entry of await self.WORKER_OBJECTS[daemonId].getOutputHistogram(
    amounts,
    minCount,
    maxCount,
    isUnlocked,
...
self.openWalletData = async function (walletId, path, password, networkType, keysData, cacheData, daemonUriOrConfig) {
  ...
  self.WORKER_OBJECTS[walletId] = await MoneroWalletFull.openWallet({
    path: "",
    password: password,
...
self.getReserveInfo = async function (walletId) {
  return (await self.WORKER_OBJECTS[walletId].getReserveInfo()).toJson();
};
